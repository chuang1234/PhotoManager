...
from config.config import (
    DB_CONFIG, UPLOAD_PHOTO_FOLDER, ALLOWED_EXTENSIONS, MAX_CONTENT_LENGTH,
...
def delete_album():
    ...
    try:
        ...
        # 删除照片物理文件
        for photo in photos:
            file_path = os.path.join(app.config['UPLOAD_PHOTO_FOLDER'], photo['file_path'])
            if os.path.exists(file_path):
                os.remove(file_path)
        # 删除照片记录
...
# 3. 修改收藏夹名称
@app.route('/api/favorite/folders/<int:folder_id>', methods=['PUT'])
@login_required
...
# 4. 删除收藏夹（非默认）
@app.route('/api/favorite/folders/<int:folder_id>', methods=['DELETE'])
@login_required
...
# 5. 照片加入收藏夹
@app.route('/api/favorite/photos', methods=['POST'])
@login_required
...
# 6. 照片移出收藏夹
@app.route('/api/favorite/photos', methods=['DELETE'])
@login_required
...
# 7. 获取收藏夹下的照片列表（分页）
@app.route('/api/favorite/photos/<int:folder_id>', methods=['GET'])
@login_required
def get_favorite_photos(folder_id):
    ...
    try:
        ...
        # 2. 查询当前页照片数据（关联用户信息）
        cursor.execute(
            '''SELECT p.*, m.name as member_name, o.name as operator_name 
               FROM favorite_photo fp 
               JOIN photo p ON fp.photo_id = p.id 
               LEFT JOIN family_member m ON p.member_id = m.id 
               LEFT JOIN family_member o ON p.operator_id = o.id 
