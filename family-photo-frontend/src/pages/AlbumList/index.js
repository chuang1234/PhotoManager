...
import { Button, Card, Modal, Input, Popconfirm, Upload, message, Space, Typography } from 'antd';
import {
    EditOutlined,
    DeleteOutlined,
    UploadOutlined,
...
const AlbumList = () => {
    ...
    const deleteAlbum = async (albumId) => {
        try {
            const res = await request.post('/api/album/delete', { album_id: albumId });
            if (res.code === 200) {
    ...
    const openCoverUpload = (albumId) => {
        ...
        const uploadInput = document.querySelector(`.${styles.hideUploadBtn} input[type="file"]`);
        if (uploadInput) {
